...
function addCell(cells, coord) {
	neighbours = neighboursKeys(coord[0], coord[1]);
	activate(cells, key(coord[0], coord[1]), neighbours);
}

function isAlive(cells, key) {
...
function countAliveFromKeys(cells, keyList) {
	let count = 0;
...
function activate(cells, key, neighbours) {
	cells[key] = true;
...
function updateCell(cells, key, status, count, neighbours) {
	let r = rules(status, count);
	if (r == -1) { // cell goes dead. Do not remove it!
		cells[key] = false;
	}
	else if (r == 1) { // cell goes live.
		activate(cells, key, neighbours);
	}
}

function cleanupCell(cells, key, status, count, neighbours) {
	if (! status && count == 0) {
		delete cells[key]; 
	}
}
...
async function run(cells, epochsNumber, cleanupCooldown, drawingFunction=null) {
	isRunning = true;
...
function rules(status, count) {
	if (status && (count < 2 || count > 3)) { // cell dies.
		return -1;
	}
	else if (! status && count == 3) { // cell lives!
		return 1;
	}
...
function createGrid(cells, x, y, width, height) {
